  ...
  _bulk (table, options, callback) {
    super._bulk(table, options, err => {
      ...
      this.parent.acquire(this, (err, connection) => {
        ...
        if (table.create) {
          const objectid = table.temporary ? 'tempdb..['+String(table.name)+']' : table.path
          ...
          if (isChunkedRecordset) {
            ...
            if (columns[JSON_COLUMN_ID] && config.parseJSON === true) {
              try {
                ...
                row = null
                const ex2 = new RequestError(new Error("Failed to parse incoming JSON. "+String(ex.message)), 'EJSON')
          ...
          if (Object.keys(this.parameters).length) {
            for (const name in this.parameters) {
              ...
              if (value instanceof TypeError) {
                value = new RequestError("Validation failed for parameter '"+String(name)+"'. "+String(value.message), 'EPARAM')
